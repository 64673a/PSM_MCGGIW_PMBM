...
m = size(W,2);                  %number of measurements 量测数量
T = model.num_iterations*m;     %number of iterations 迭代次数

%Initialisation, each measurement corresponds to a new object 初始化，每个测量值对应一个新对象
meas_cell = cell(m,1);
Lc = zeros(m,1);                %likelihood for each cell 每个单元的可能性
for i = 1:m
    meas_cell{i} = i;
