classdef measmodel
    ...
    methods (Static)
        function obj = cvmeasmodel(sigma)
            ...
            %INPUT:     sigma: standard deviation of measurement noise --- scalar 量测噪声的标准偏差
            %OUTPUT:    obj.d: measurement dimension --- scalar 计量尺寸
            %           obj.H: function handle return an observation matrix --- 2 x 4 matrix 观察矩阵
            %           obj.R: measurement noise covariance --- 2 x 2 matrix 量测噪声协方差
            %           obj.h: function handle return a measurement --- 2 x 1 matrix 返回量测值
            obj.d = 2;
            obj.H = @(x) [1 0 0 0;
                                    0 1 0 0];
            obj.R = sigma^2*eye(2);
