function [partitions,lik] = ObjectsSO_new1(tracks_L,~,PPP,W,gating_matrix_d,gating_matrix_u,model)
...
n_tt = length(tracks_L);               %number of tracks in the jth global hypothesis
gating_matrix_dj = zeros(m,n_tt);           %extract gating matrix for each track
for i = 1:n_tt
    gating_matrix_dj(:,i) = gating_matrix_d{i};
    tracks(i,1) = tracks_L{i}(1);
end
...
%Pre-computing misdetection likelihood 预计算错误检测可能性
Lmiss = zeros(n_tt,1);
for i = 1:n_tt
    [~,Lmiss(i)] = misdetectionBern(tracks(i,1).Bern,model,length(model.motionmodel.F));
end
 
%Maximum likelihood initialisation 最大可能性初始化
nu = length(PPP.w);
...
for i = 1:n_tt
    ...
    Shat=[0 0
          0 0];
    for f=0:length(model.motionmodel.F)-1
        ghat = ghat+tracks(i,1).Bern.GGIW(end-f).a/tracks(i,1).Bern.GGIW(end-f).b;
        zhat = zhat+model.measmodel.h(tracks(i,1).Bern.GGIW(end-f).m);
        Shat = Shat+model.measmodel.H(tracks(i,1).Bern.GGIW(end-f).m)*tracks(i,1).Bern.GGIW(end-f).P*model.measmodel.H(tracks(i,1).Bern.GGIW(end-f).m)' ...
            + model.measmodel.R + tracks(i,1).Bern.GGIW(end-f).V/(tracks(i,1).Bern.GGIW(end-f).v-2*2-2);
        
    end
    ghat=ghat/length(model.motionmodel.F);
    zhat=zhat/length(model.motionmodel.F);
    Shat=Shat/length(model.motionmodel.F);
    Shat = (Shat + Shat')/2;
...
for i = 1:n_tt
    if isempty(meas_cell{i})
        ...
        [~,Lp(i)] = detectionBern(tracks(i,1).Bern,W(:,meas_cell{i}),model,length(model.motionmodel.F));
    end
end
for i = n_tt+1:N
...
for t = 1:T
    ...
    if cell_idx <=n_tt %selected measurement from a target cell
        if isempty(mea_after_move)
            ...
            [~,Lp_move2] = detectionBern(tracks(cell_idx).Bern,W(:,mea_after_move),model,length(model.motionmodel.F));
        end
        ...
        if i==cell_idx
            ...
            if i <= n_tt
                if gating_matrix_dj(mea_selected,i)
                    [~,Lp_move1(i)] = detectionBern(tracks(i).Bern,W(:,[meas_cell{i} mea_selected]),model,length(model.motionmodel.F));
                end
            else
...
%                         [~,Lp_merge(i)] = detectionBern(tracks(i).Bern,W(:,[meas_cell{cell_idx} meas_cell{i}]),model,length(model.motionmodel.F));
%                     end
...
%                     [~,Lp_split1(1)] = detectionBern(tracks(cell_idx).Bern,W(:,meas_cell{cell_idx}(IDX==1)),model,length(model.motionmodel.F));
...
%                     [~,Lp_split1(2)] = detectionBern(tracks(cell_idx).Bern,W(:,meas_cell{cell_idx}(IDX==2)),model,length(model.motionmodel.F));
