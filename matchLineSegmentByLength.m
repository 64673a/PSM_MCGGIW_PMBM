function [LineSegmentSet,sumCov]=matchLineSegmentByLength(LineSegmentSet,LineSegment,GGIW,model,time,lambda_l)
...
if ~isempty(LineSegment)
    ...
    for l=1:length(matchLineSegment) %Check occlusion
    ...
    for i=1:length(LineSegment)
        ...
        for j=1:length(matchLineSegment)
            lineBangle=k2angle(matchLineSegment{j})+theta*1.4;
            ...
            if (lineD>lambda_d && abs(lineAangle-lineBangle)<lambda_a && isOcclusion(j)~=1 && L<norm(v)*lambda_l)
                if matchLength(1,i)==0
                    ...
                    %Match the two closest lines
                    if sum(abs((LineSegment{i}.start+LineSegment{i}.end)/2-(matchLineSegment{j}.start+matchLineSegment{j}.end)/2))<sum(abs(matchLength(1:2,i)))
                        matchLength(1:2,i)=(LineSegment{i}.start+LineSegment{i}.end)/2-(matchLineSegment{j}.start+matchLineSegment{j}.end)/2;
                        matchLength(3,i)=sum(abs((LineSegment{i}.start+LineSegment{i}.end)/2-(matchLineSegment{j}.start+matchLineSegment{j}.end)/2));
                        matchLength(4,i)=i;
                        matchLength(5,i)=j;
    ...
    if ~isempty(matchLength)    
        ...
        matchEnd=0;
        v_sum=0;
        for i=find(matchLength(1,:))%Judging by covariance
        ...
        headNewLine=0;
        tailNewLine=0;
        if ~isempty(find(matchLength(5,1:matchEnd)==1, 1)) && LineSegmentSet.Closed==0%head
        ...
        if ~isempty(find(matchLength(5,1:matchEnd)==length(LineSegmentSet.LineSegment), 1)) && LineSegmentSet.Closed==0%tail
        ...
        for i=1:headNewLine %Assignment (header)
        ...
        for i=1:length(new_LineSegment) %Assignment (old)
        ...
        idx=0;
        for i=tailNewLine:-1:1 %Assignment (tail)
            idx=idx+1;
        ...
        for i=1:length(LineSegmentSet.LineSegment)
            if length(GGIW)+1-LineSegmentSet.LineSegment{i}.birthTime==3 && LineSegmentSet.LineSegment{i}.matchCount<=lmbda_m
                deletLineTable(i)=1;
    ...
    else%If all extensions do not match
